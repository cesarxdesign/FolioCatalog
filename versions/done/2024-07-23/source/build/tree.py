...
def walk(n,x,y,d,sx,sy):
    print('  '*d+f"{n.tag[:4]} {n.get('id')} '{(n.get('name') or '')[:45]}' rel({x-sx:g},{y-sy:g}) {float(n.get('width')):g}x{float(n.get('height')):g}"+(' HID' if n.get('hidden') else ''))
    if d<maxd:
        for c in n: walk(c,x+float(c.get('x',0)),y+float(c.get('y',0)),d+1,sx,sy)
for i in ids:
    n,x,y=find(t,0,0,i)
    # section-relative: find section ancestor = absolute y; print coords relative to section start (sections are at frame level)
    sec=None
    for s in t:
        sy=float(s.get('y')); 
