...
def run(out_png, js_files=(), hash_='#1x'):
    ...
    try:
        ...
        ev("""new Promise(r=>{const go=()=>document.fonts.ready.then(()=>{
              ...
              Promise.all(imgs.map(i=>i.complete&&i.naturalWidth?1:new Promise(res=>{i.onload=i.onerror=res}))).then(()=>Promise.all(imgs.map(i=>i.decode().catch(()=>0)))).then(()=>setTimeout(r,600))});
