...
@font-face{{font-family:WMSerif;src:url(bodoni72-book-starcount.woff2)}}
...
<span style="font-family:WMSans;font-weight:{p['w3']};font-size:{p['s3']}px;letter-spacing:{p['l3']}px;left:{p['x3']}px;top:{p['y3']}px">OBSERVATORY</span>
    ...
    open('wm.html','w').write(html)
    subprocess.run(['./render.sh',S+'/fit/wm.html',S+'/fit/wm.png','980','560'])
    a=np.asarray(Image.open('wm.png').convert('L')).astype(int)[:1103]
    return boxes(a),a
