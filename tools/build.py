...
#sort,#lock,#theme{display:flex;align-items:center;gap:6px;padding:4px 10px;font-size:12px}
#sort{margin-left:auto}
#sort:disabled{opacity:.35;cursor:default}
#lock[aria-pressed="true"]{border-color:var(--accent);color:var(--accent)}
#sort svg,#lock svg,#theme svg{width:14px;height:14px}
...
.cap{flex:none;display:flex;gap:4px;padding:5px 6px;border-bottom:1px solid var(--line)}
...
<header>
  ...
  <button type="button" id="sort" title="Put the pages shown in order, earliest on the left"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 8h14M13 4l4 4-4 4M21 16H7M11 12l-4 4 4 4"/></svg>Resort</button>
  <button type="button" id="lock" aria-pressed="true" title="Scroll every pane together"></button>
...
const state = {project: projects[0], version: '', n: 1, off: 0, picks: []};
...
// On unless this browser turned it off. (A new key: the old one stored "off" on every visit.)
let locked = store.get('fc-scroll-lock') !== '0';
...
function applyLock() {
  store.set('fc-scroll-lock', locked ? '1' : '0');
...
function defaults() {
  // One project, or one version name: the latest n, oldest on the left. All: each project's live page, A-Z.
  const vs = pool();
...
function fill() {
  ...
  // Whenever the viewer fills panes itself, they run oldest to newest, left to right (ALL is
  // in that order). A page picked in a pane's caption stays where it was put.
  if (picks.length > had) picks.sort((a, b) => ALL.indexOf(byKey[a]) - ALL.indexOf(byKey[b]));
  state.picks = picks;
}
...
function pickerFor(i) {
  ...
  if (state.project === 'all') {
    for (const p of projects) {
      const vs = pool().filter(v => v.project === p);
      if (vs.length) s.append(el('optgroup', {label: cap(p)}, ...vs.map(opt)));
    }
...
function closerFor(i) {
  ...
  b.setAttribute('aria-label', `Remove pane ${i + 1}`);
  b.hidden = state.picks.length < 2;
  b.onclick = () => { state.picks.splice(i, 1); state.n = state.picks.length; render(); };
  return b;
}
...
function render() {
  ...
  $('sort').disabled = set.every((k, j) => !j || byTime(set[j - 1], k) <= 0);
  ...
  fitFrames();
  if (locked) equalize();
  const hash = `#${state.project}/${state.n}/${state.off}/${set.join(',')}` + (state.version ? '/' + state.version : '');
...
if (!ALL.length) {
  ...
  // Every page in the view, not just the ones on screen, earliest to latest, left to right.
  $('sort').onclick = () => { state.picks.sort(byTime); render(); };
  applyTheme();
  applyLock();
  readHash();
  render();
